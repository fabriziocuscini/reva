/**
 * One-shot script: converts Tailwind CSS v4 OKLCH colour values to hex
 * and writes the result to src/data/tailwind-colors.json.
 *
 * Run: bun run scripts/generate-tailwind-colors.ts
 */
...
for (const [name, steps] of Object.entries(tw)) {
  result[name] = {}
  for (const [step, oklchStr] of Object.entries(steps)) {
    const parsed = parse(oklchStr)
    if (!parsed) {
      console.warn(`Failed to parse ${name}-${step}: ${oklchStr}`)
      continue
    }
    const clamped = clampChroma(parsed, 'oklch')
    const hex = formatHex(clamped)
    result[name][step] = (hex ?? '#000000').toUpperCase()
  }
}

const outPath = new URL('../src/data/tailwind-colors.json', import.meta.url).pathname
await Bun.write(outPath, JSON.stringify(result, null, 2) + '\n')
console.log(`Wrote ${Object.keys(result).length} palettes to ${outPath}`)
